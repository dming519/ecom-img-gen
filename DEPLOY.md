...
1. push 到 `main`
2. 在 GitHub Actions 页面手动执行 `Deploy`
...
2. `npm run check`
3. `npm run build`
4. `npx wrangler pages deploy dist --project-name ecom-img-gen`
...
如果 Cloudflare 后续提示权限不足，再按错误提示补充对应资源的 Edit 权限。Cloudflare Pages 项目 `ecom-img-gen` 是 Direct Upload 项目，Cloudflare API 不允许把现有 Direct Upload 项目原地改成 Git Provider；因此这里用 GitHub Actions 监听 push 并部署到同一个 Pages 项目。
...
常规发布由 GitHub Actions 自动执行；上面的命令仅作为本地备用手动发布。
