...
function createEnhancedPrompt(prompt: string, aspectRatio: AspectRatio, quality: ImageQuality) {
  return [
    "产品参考图是事实来源：画面中凡是出现完整产品、包装、Logo、标签、局部结构、材质纹理或颜色，都必须与上传参考图保持一致。",
    "不要重新设计、重绘、改造或替换产品；不要改变瓶型、瓶盖高度和直径、瓶身宽高比例、版型、接口、肩线、底部弧度、包装材质、主色、标签版式、品牌 Logo、文字位置和图案位置。",
    "允许根据单张详情图主题只提取参考图里的真实特征进行表达，例如材质纹理、包装色彩、局部工艺、标签元素、结构细节、使用状态、参数信息或场景氛围；不要求每张图都展示完整产品本体。",
    "如果该张图展示完整产品，只能在不改变产品本体的前提下优化背景、光影、陈列环境和周边信息模块；如果参考图是矮胖瓶、宽瓶盖、半透明浅紫瓶身，就不能生成高瘦瓶、细瓶盖或不同包装。",
