...
const modules = [
  ...
  },
  {
    ...
    detail: "详情图模块把商品名称、核心卖点、功效说明和参考图组合成多张详情页内容，适合电商商品页、落地页和私域转化页。文案和图片可以逐张调整，支持重新生成当前图。",
    benefits: ["批量产出详情页", "图文结构更完整", "支持逐张重抽"],
    before: "设计、文案、修图分散协作，详情页从策划到出图周期长。",
    after: "一套商品资料生成多张详情图，文案、图片和历史记录集中管理。",
    icon: "spark" as const,
    href: "/image/",
    visual: "detail",
    cta: "生成商品详情图",
  ...
  },
  {
    ...
    after: "上传参考图并选择张数，系统自动产出统一白底的多视角商品素材。",
    icon: "queue" as const,
    href: "/multi-view/",
    visual: "angles",
    cta: "进入多视角模块",
  ...
  },
  {
    title: "分层",
    label: "PNG 图层包",
    desc: "把图片拆成商品、背景、文案和装饰层，打包下载透明 PNG 图层。",
    detail: "分层模块用于把已经生成或上传的图片拆成可操作素材层，输出透明 PNG 图层包和图层清单，让设计师可以替换背景、调整商品、修改文字和二次排版。",
    benefits: ["透明 PNG 图层", "图层结构清晰", "ZIP 打包下载"],
  ...
  <main class="app-shell home-shell">
    ...
    <section class="home-hero home-hero-premium">
      <div class="home-hero-copy">
        ...
        <p>
          EcomImgGen 为商品运营、设计和投放团队提供统一的 AI 创作工作台，覆盖商品主图、详情图、白底图、多视角图、PNG 分层和商品介绍视频，让商品素材生产从零散工具升级为连续工作流。
        </p>
    ...
    <section class="home-detailed-modules" aria-label="模块详细介绍">
      ...
      >
        <div class="module-detail-copy">
          ...
          <button v-else class="btn-secondary module-detail-cta" type="button" disabled>
            {{ item.cta }}
          </button>
        </div>
