...
- [ ] 已创建 EcomImgGen 专用 `TASKS_KV`
- [ ] 根目录 `wrangler.toml` 已写入同一个 KV ID
- [ ] `worker/wrangler.toml` 已写入同一个 KV ID
...
- [ ] 已创建 Pages 项目 `ecom-img-gen`
- [ ] Build command 是 `npm run build`
- [ ] Output directory 是 `out`
- [ ] 已绑定 `TASKS_KV`
- [ ] 已配置 OAuth Secrets
- [ ] 已配置 OpenAI-compatible 接口 Secrets
- [ ] 已配置 `IMAGE_WORKER_URL`
- [ ] 已配置 `IMAGE_WORKER_TOKEN`
- [ ] Pages 和 Worker 的 `IMAGE_WORKER_TOKEN` 完全一致
